...
### Live Stat Tracker (v2)
Built: `website/stats.html` — a full-screen, tablet-kiosk companion page for a dedicated stats keeper, separate from the ref-facing scoreboard (no site nav, no pinch-zoom, a Fullscreen button). The "Now on the Scoreboard" panel detects whatever match the app is currently running over the public relay feed (same URL the phone scoreboard uses) and one tap creates the session and jumps into lineup setup — one team at a time, 3 bots + drivers each. During play: on-ice bots show 6-at-a-glance with each bot's own identifying color (set per-bot on the team page) as a name badge; **tapping an on-ice bot logs a hit immediately, tapping a bench bot starts a sub** (no separate buttons/modals for either); goals still auto-pop a scorer/assist/own-goal picker off the relay's live score feed, with a manual fallback button. Every event is stamped with the **game clock** (e.g. "9:40"), not wall-clock time. A one-level **Undo Last** button reverses the most recent tap (including un-doing a sub). Nothing is trusted automatically — post-game review confirms/edits/deletes each event before "Validate All." Two Supabase tables (`stat_sessions`, `game_events` — now with `game_clock`), migrations 011+012 run; no app or relay changes. Not yet: public surfacing of validated stats anywhere on the site.
