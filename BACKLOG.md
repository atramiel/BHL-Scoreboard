...
Built: `Services/DiscordService.cs` posts to a Discord webhook (`DiscordWebhookUrl` in Settings) — no OAuth, fire-and-forget, no retry queue (a missed hype post has no record-keeping stakes). **Final score** posts for every game, exhibition included — Discord is hype/community, not a permanent record, so it isn't gated on a selected match (unlike Challonge/league-site reporting). **Next Up** posts the moment a match slot is short-tapped on the Stream Deck (selecting it to run next), text reads "Next Up: A vs B — Load In". **Sudden Death** hype pings are match-linked only, keeping "come watch this" urgency for real bracket games. Championship finals get a gold-colored embed instead of blue; final-score embeds use the winning team's public logo as a thumbnail (reusing the same Supabase-hosted URLs the league site already uses). A manual **Post Recap to Discord** button (Settings) lists today's completed games, since there's no "the event is over" signal in the app to trigger it automatically. Post types toggleable individually (Final Scores / Next Up / Hype Pings / On Deck — see below).

### Discord "On Deck" Ping
Built: long-pressing (500ms+) a match-slot button on the Stream Deck posts a manual "get ready" ping tagging just the two teams in that slot — a deliberate operator action, not automatic, since BHL often runs matches back-to-back with no gap worth announcing. Short tap still just selects the match (posts "Next Up") without tagging. Both presses flash the Stream Deck's native green checkmark (`showOk()`) the instant the command fires, so there's a visual confirmation something was actually sent. Role IDs are admin-managed on the website (`admin.html` → "Team Discord Role"), not team-editable — `discord_role_id` is deliberately excluded from the public `teams_public` view/API, only readable via the admin-key-gated `admin_get_discord_roles()` RPC. Resolves the same `team_aliases` chain the app already uses for logos (e.g. Challonge's "Malice" → website's "Team Malice"), so a team doesn't need its Challonge name to exactly match its website name. Requires `website/migrations/014-discord-role.sql`.
...
### Attract Mode: Overflow Auto-Scroll, Full-Height Panels, Today's Results Second Logo
Built: long Team Spotlight bios/rosters (e.g. EVAC, BlueShift) were getting clipped top-and-bottom or cut off mid-content on the between-game attract carousel. Fixed with a real `ScrollViewer` per side (left/right) instead of a hand-rolled transform+manual-height approach that kept losing races against WPF's own layout timing — `ScrollViewer.ScrollableHeight` is tracked natively, so the scroll target is recomputed every ~30ms tick against the live value instead of measured once and locked in. Content shorter than the viewport centers automatically (`VerticalContentAlignment`); taller content holds 2s, then scrolls at a constant ~40px/s reading pace, holding the between-game screen on that panel long enough to actually finish (up to a 90s safety ceiling) instead of getting swapped away mid-scroll. Left/right panels now span the full screen height (including the row beside the logo, which was otherwise unused) instead of a narrower fixed box. Row spacing tightened slightly so an 8-team Live Rankings panel fits without needing to scroll at all. Today's Results now shows both teams' logos (winner + loser), not just the winner's — same two-logo treatment as Upcoming Matches.

### Pre-Game Speech Screen
Built: a new Stream Deck button opens a full-screen, crowd-facing slide deck the operator pages through before a game — welcome, sponsors, rules, safety reminders, then each team and the audience individually, ending on "count down with me." Slides are Settings-editable (`---`-separated), with placeholder tokens: `{home}`/`{away}` show that team's name + logo + tint the background to their configured scoreboard color; `{sponsors}` shows uploaded sponsor logos; `{branding}` shows the BHL crest + a configurable event logo; `{caution}`/`{hype}` pick a matching confetti glyph set (⚠️🚧 vs 🔥📣) without changing anything else. Each slide can also show up to 4 GIFs, randomly picked, stored under `Resources/PreGameSlideGifs/SlideN/` (git-tracked, not the gitignored build output) so they travel with the repo — currently populated per-slide-position, not content, so reordering slides in Settings shifts which GIF folder applies. The press made while "count down with me" is already on screen closes the overlay and starts the game immediately (same as Play/Pause on a fresh game), skipping the between-game lead-in timer entirely. Sponsor logos and the event logo are uploaded via Settings (file picker, copied locally).

### Multi-Bracket Match Selection
Built: the Stream Deck's match-slot buttons now pull open matches from the main bracket AND any "Other Tournaments" brackets (previously display-only), merged and capped at 6. Each match remembers which bracket it came from, so reporting a finished result posts back to the correct bracket instead of always assuming the main one.
...
Nothing currently queued — the last item (Pre-Game Speech) moved to "In Testing" above. See "Reliability & Plumbing" below for lower-priority ideas not yet scheduled.
...
### Settings Backup & Export — Cloud Half Still Open
Local half built: Settings → "Export All Settings..." bundles `gameSettings.json`, `bindings.json`, and uploaded sponsor/event logos into one `.zip`; "Import Settings..." restores it on another machine, re-rooting logo paths to that machine's own folder rather than trusting the old absolute paths. Not done: storing the same bundle on the league website so a laptop can pull it without needing the zip file physically handed over.
